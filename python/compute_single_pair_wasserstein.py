...
from compute_two_point_wasserstein import get_number_of_samples
...
def compute_wasserstein_one_point(file_a, file_b, multi_x, multi_y, normalization_file=None,
    ...
    if normalization_file is not None:
        number_of_normalization_samples = get_number_of_samples(normalization_file)
        normalization_data = load_file_mean(normalization_file,
                                            conserved_variables,
                                            number_of_normalization_samples)


        # Want the L1 norm of the mean
        normalization_factor = np.sum(abs(normalization_data), axis=(0,1))/normalization_data.shape[0]**2
    
                                            

    if use_all_samples:
        number_of_samples = get_number_of_samples(file_a)
        assert(number_of_samples == get_number_of_samples(file_b))
    else:
        number_of_samples = resolution
